    ...
    <table style="table-layout: auto;" class="table table-striped table-bordered">
        ...
        <tbody>
            @foreach ($categories as $index => $item)
                <tr>
                    <td class="text-center">{{ $categories->currentPage() * $categories->perPage() - $categories->perPage() + $index + 1 }}</td>
                    <td>
                        <div class="parent_id">
                    ...
                    <td>
                        <span><img class="img-fluid" src="{{ asset('uploads/products/product_categories/' . $item->image) }}" alt=""></span>
                    </td>
                    <td>
                        {{ $item->description }}
                    </td>
                    <td class="text-center">
                        <label class="toggle">
                            <input id="toggleswitch" class="toggleswitch" name="publish" type="checkbox"
                                value="{{ $item->publish }}" data-id="{{ $item->id }}" data-model="ProductCategory"
                    ...
                    <td>
                        <div class="list-icon-function">
                            @if ($config == 'deleted')
                                ...
                                <form class="form-delete" action="{{ route('productCategory.forceDelete', $item->id) }}" method="POST">
                                    @csrf
                                    @method('DELETE')
                            ...
                            @else
                                <a href="{{ route('productCategory.edit', $item->id) }}" title="Chỉnh sửa">
                                    <div class="item edit">
                                        <i class="icon-edit-3"></i>
                                    </div>  
                                </a>
                                <form class="form-delete" action="{{ route('productCategory.destroy', $item->id) }}" method="POST">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="btn btn-delete item text-danger delete" title="Xóa"
